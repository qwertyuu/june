...
bool june::InitLLVMNativeTarget() {
...
llvm::TargetMachine* june::CreateLLVMTargetMache() {
...
void june::SetTargetToModule(llvm::Module& LLModule, llvm::TargetMachine* LLTargetMachine) {
	auto TargetTriple = llvm::sys::getDefaultTargetTriple();
	LLModule.setTargetTriple(TargetTriple);
	LLModule.setDataLayout(LLTargetMachine->createDataLayout());
}

bool june::WriteObjFile(const c8* FilePath, llvm::Module& LLModule, llvm::TargetMachine* LLTargetMachine) {
	...
	if (LLTargetMachine->addPassesToEmitFile(Pass, OSFileDest, nullptr, llvm::CGFT_ObjectFile)) {
		// TODO Logger::GlobalError(llvm::errs(), "TheTargetMachine can't emit a file of this type");
		return false;
	}
